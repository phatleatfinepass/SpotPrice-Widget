...
test_output="$(mktemp -d "${TMPDIR:-/tmp}/spotprice-uninstaller-target-tests.XXXXXX")"
...
"$swiftc_path" \
  ...
  "$repo_root/Tests/UninstallerTargetTests.swift" \
  -o "$test_output/UninstallerTargetTests"

"$test_output/UninstallerTargetTests"
